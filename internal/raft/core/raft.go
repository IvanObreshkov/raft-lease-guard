...
type RaftPersistedState struct {
	...
	VotedFor *ServerID
	// Log holds the log entries; each entry contains a command for the state machine and the term when the entry was
	// received by the leader. The first index is 1, as per Figure 2 and Section 5.3 from the
...
// Raft is the complete state of a Raft server: the persistent and volatile state that every server keeps, the volatile
